...
Modified by D. Djokic 2011 - to fit workflow. 
...
def rename_jpeg_file(fn,txt):
    ...
    while exists:
        if index == 0:
            new_name = txt+'-'+prefix + ext # + '_' + base
        else:
            new_name = txt+'-'+prefix + '_' + str(index) + ext
...
def rename_jpeg_files_in_dir(dn, txt):
    names = os.listdir(dn)
    count=0
    for n in names:
        file_path = os.path.join(dn, n)
        count += rename_jpeg_file(file_path, txt)
    return count


if __name__=='__main__':
	text=raw_input("Input textual prefix: ")
	path=raw_input("Input full directory path: ")
	#try:
	#	path = sys.argv[1]
	#except IndexError:
	#	print '''Usage:  
...
if os.path.isfile(path):
	rename_jpeg_file(path, text)
elif os.path.isdir(path):
	count = rename_jpeg_files_in_dir(path, text)
